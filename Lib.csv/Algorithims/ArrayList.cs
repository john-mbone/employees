...
namespace Algorithms
{
    ...
    public class ArrayList<T> : IEnumerable<T>
    {
        ...
        ///Get Range To List
        ///Returns GetRange cast to List

        public List<T> GetRangeToList(int startIndex,int count)
        {
            return GetRange(startIndex: startIndex, count: count).ToList();
        }

        public int SearchInArrayIndex<T>(T value,int index)
        {
            var arrayList = new ArrayList<T>(Count);

            return Array.BinarySearch(_collection, index, Count, value);
        }
